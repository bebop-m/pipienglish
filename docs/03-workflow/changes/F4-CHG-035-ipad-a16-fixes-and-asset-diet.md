...
## 3c. 2026-09-11 深夜追加(小皮:「贴纸拖到今日任务板周围或下面有空气墙」)

- 原保留区是任务卡与完成卡的并集矩形(到 y=457),并要求物件整张离开矩形;任务卡本身只到 339,卡片下方一大片和右侧一条都成了空气墙。
- 改为两条:① 只按当前真正显示的那张卡片算(任务/完成/起名/回访四张实测外框写在 `HOME_BOARD_RECTS`,由 `boardKindFor` 按首页状态选);② 软规则 `blockedByKeepouts`:露出 ≥35% 面积且不少于 44×44pt 就允许停下,贴纸可以贴着卡片、伸到卡片下面,只有整张(或几乎整张)藏起来才被就近挪出(`settleOutsideKeepouts` 按 8pt 步长四方向搜索,上限整个舞台宽度)。
- 贴纸、母鸡/小皮/小鸡、鸡窝/救援框、固定装置全部走同一套规则;角色落点另加鸡窝可见图与救援框当前位置。VM 暴露 `uiKeepouts`,落库(placeDecoration / placeSceneElement)与读取(snapshot)按当天会话状态算同一份。
- 浏览器实测(任务卡态):花篮放到卡片正下方 (199,421) 原地不动、25/25 可点;风车靠在卡片右侧 (471,374) 21/25 可点;花篮整张塞进卡片背后被最小距离挪到 (199,357),10/25 可点。
