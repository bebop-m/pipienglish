...
1. 完成当日必修 → `eggStock += 1`(蛋经济 v2,2026-07-17 爸爸定稿:固定 1 枚;写词游戏另供 ≤5 枚/日;同日幂等)。
...
| 层 | 工具 | 覆盖 | 责任 |
|---|---|---|---|
| domain | Vitest 纯单测 | 蛋经济规则(发蛋幂等/分配守卫/24h 孵化)、dailyPlan(≤6/最过期优先)、staleness 权重分布、streak、翻转结算 | Claude |
...
first_visit / daily_complete 正式 P0 位图替换与视觉锁定 / rescue ×0 态 / 40+ 小鸡满员表现 /
