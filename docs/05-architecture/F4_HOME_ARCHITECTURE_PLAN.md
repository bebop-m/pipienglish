...
6. 场景贴纸每章价格 90，角色装扮完整目录每章 80；购买、收藏替换、摆放与装备均以原子事务维护库存和所有权。无真实货币、限时、折扣或属性收益。
...
3. 初始化 `activeSceneId='scene-1'`、`acknowledgedSceneChapter=1`、两个保底计数器为 0、`cookingMeal=null` 和默认 `CharacterLoadout`。
...
导出版本升级为 **v3**，包含场景、小鸡稀有外观与收藏、贴纸与坐标、衣柜与装备、料理和章节合照。导入兼容 v1/v2/v3，旧版本统一经过 §6.3 转换；Date 字段序列化/反序列化规则沿用现实现。家长页每周导出提醒不变(SPEC §6 硬需求)。
...
| application | Vitest + fake-indexeddb | v2→v3 零损失迁移、放蛋时原子定外观、收藏轮换、料理崩溃与旅行退款、内容脱期兜底、贴纸/衣柜交易、v1/v2/v3 备份导入 | Claude |
