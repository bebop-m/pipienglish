...
- **阶段 G · PWA 收口与部署(2026-07-17 爸爸定:GitHub Pages)**
  ...
  **状态:🟡 2026-07-17 PWA 核心收口完成，云存储方案待爸爸裁决。**manifest 已固定
  ...
  经 GitHub 官方文档复核，公开仓库的任一分支都不能作为私密学习数据存储；因此不得把明文备份
  写入现有公开 `backups` 分支。推荐另建私有备份仓库并使用仓库级 fine-grained PAT；在爸爸选择
  私有仓库或客户端加密方案前，云备份暂停，本地 JSON 导出/导入继续保留。详见 `F4-CHG-006`。
...
等待 A2759 standalone 真机验收与云备份存储方案裁决。
