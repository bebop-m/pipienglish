...
> **适配范围裁决(2026-07-17 爸爸)**:当前只适配 **iPad Pro 11 英寸(A2759)横屏**
> ——小皮的实际设备,原生即 1194×834。本文中涉及其他尺寸(13 英寸、竖屏、
> Split View)的适配与回归项(IP-13 等)全部**推迟到完整版正式上线后**再执行;
> 固定舞台的缩放公式保持不变,届时直接复用。
>
> 已完成能力、历史验证和后续恢复顺序统一记录在
> [`F4_MULTI_SIZE_ADAPTATION_STATUS.md`](F4_MULTI_SIZE_ADAPTATION_STATUS.md)。
...
<div class="f4-viewport">
  ...
  <main class="f4-stage" aria-label="皮皮のEnglish 小鸡农场">
    <!-- F4 的全部背景、UI、角色与弹层 -->
  </main>
...
下表同时保留当前发布门槛和未来多尺寸用例。标为“延期”的历史截图只证明固定舞台外壳可行，
不代表真实完整产品已经支持该尺寸。

| 用例 | 视口/模式 | 验证内容 | 当前范围 |
|---|---|---|---|
| GM-01 | 1194×834，DPR 1 | 与 F4 黄金母版逐像素比较 | 当前发布门槛 |
| IP-11 | A2759 横屏，standalone 等效视口 | 核心构图、字号、命中区、安全区 | 当前发布门槛；最终真机验收待完整产品完成 |
| IP-13 | iPad Pro 13 横屏，1366×1024 | 核心舞台不变，额外区域只有背景延展 | 延期；仅阶段 B 外壳预验证 |
| PORTRAIT | 834×1194 示例 | 隐藏舞台并显示横屏引导 | 当前仅验证保护提示，不是竖屏适配 |
| SPLIT | 800×500 示例 | `scale < 0.72` 时显示全屏提示 | 延期；仅阶段 B 外壳预验证 |
| RM | 1194×834，减少动态 | 所有信息可用，无角色持续移动 | 当前发布门槛 |
| OFF | A2759 等效视口，断网 + 冷启动 | 所有 F4 图片、CSS、JS/React 与字体可加载 | 当前发布门槛，最终整包复验待完成 |
| DRAG | A2759 横屏 | 拖拽坐标、孵化棚上方、边缘和避障区无空气墙 | 当前发布门槛；其他尺寸延期 |
...
## 10. 当前上线 Definition of Done（仅 A2759 横屏）

- F4 母版哈希校验通过，母版未被覆盖。
- 1194×834 黄金截图的元素位置、尺寸、层级、换行与确认稿一致。
- A2759 横屏 standalone PWA 中核心舞台保持 1194×834 CSS pt，无内部响应式重排。
...
13 英寸、其他 iPad、竖屏和 Split View 不属于本轮 DoD。完整版上线后扩大范围时，按
[`F4_MULTI_SIZE_ADAPTATION_STATUS.md`](F4_MULTI_SIZE_ADAPTATION_STATUS.md) §4 恢复验证；
每个新设备都必须建立完整页面矩阵并完成对应真机验收。
