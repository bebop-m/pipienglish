...
本轮不是自由创作。先确认当前任务只要求一张校准资产，实际打开视觉系统 §9.1 要求的最小锚图，写清身份/画风/环境/构图参考、允许变化、必须保留、透明画布、锚点和禁用元素。提交给生成工具前先复述每张参考图的唯一职责；场景图不得替代角色身份锚图。只有爸爸/小皮批准该单张校准后，才可冻结分层交付清单并批量生产、登记；候选不得接入生产代码。
